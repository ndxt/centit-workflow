define(function(require) {
	...
	var UserUnits = Page.extend(function() {
		...
		this.load = function(panel, data) {
			...
			form.form('load', data);
			Core.ajax(Config.ContextPath+'service/flow/useroptmgr/userunits/'+data.userCode, {
				method: 'get'
			}).then(function(data) {
				table.cdatagrid({
					controller: this
		...
		this.submit = function(panel, data, closeCallback) {
			...
			if (form.form('validate') && table.cdatagrid('endEdit')) {
				...
				data._method = 'PUT';
				Core.ajax(Config.ContextPath+'service/flow/define/'+data.version+'/'+data.flowCode, {
					data: data,
					method: 'post'
				}).then(closeCallback);
