...
CREATE  VIEW f_v_userroles AS select b.ROLE_CODE AS ROLE_CODE,b.ROLE_NAME AS ROLE_NAME,b.IS_VALID AS IS_VALID,'D' AS OBTAIN_TYPE,b.ROLE_TYPE AS ROLE_TYPE,b.UNIT_CODE AS UNIT_CODE,b.ROLE_DESC AS ROLE_DESC,b.Create_Date AS CREATE_DATE,b.update_Date AS UPDATE_DATE,a.USER_CODE AS USER_CODE,NULL AS INHERITED_FROM from (f_userrole a join f_roleinfo b on((a.ROLE_CODE = b.ROLE_CODE))) where ((a.OBTAIN_DATE <= now()) and (isnull(a.SECEDE_DATE) or (a.SECEDE_DATE > now())) and (b.IS_VALID = 'T')) union select b.ROLE_CODE AS ROLE_CODE,b.ROLE_NAME AS ROLE_NAME,b.IS_VALID AS IS_VALID,'I' AS OBTAIN_TYPE,b.ROLE_TYPE AS ROLE_TYPE,b.UNIT_CODE AS UNIT_CODE,b.ROLE_DESC AS ROLE_DESC,b.Create_Date AS CREATE_DATE,b.update_Date AS UPDATE_DATE,c.USER_CODE AS USER_CODE,a.UNIT_CODE AS INHERITED_FROM from ((f_unitrole a join f_roleinfo b on((a.ROLE_CODE = b.ROLE_CODE))) join f_userunit c on((a.UNIT_CODE = c.UNIT_CODE))) where ((a.OBTAIN_DATE <= now()) and (isnull(a.SECEDE_DATE) or (a.SECEDE_DATE > now())) and (b.IS_VALID = 'T')) ;


DROP VIEW IF EXISTS f_v_optdef_url_map;
CREATE  VIEW f_v_optdef_url_map AS select concat(c.opt_url,b.OPT_URL) AS opt_def_url,b.OPT_REQ AS opt_req,b.OPT_CODE AS opt_code from (f_optdef b join f_optinfo c on((b.Opt_ID = c.Opt_ID))) where ((c.Opt_Type <> 'W') and (c.opt_url <> '...') and (b.OPT_REQ is not null)) ;


DROP VIEW IF EXISTS f_v_opt_role_map;
CREATE  VIEW f_v_opt_role_map AS select concat(c.opt_url,b.OPT_URL) AS opt_url,b.OPT_REQ AS opt_req,a.ROLE_CODE AS role_code,c.Opt_ID AS opt_id,b.OPT_CODE AS opt_code from ((f_rolepower a join f_optdef b on((a.OPT_CODE = b.OPT_CODE))) join f_optinfo c on((b.Opt_ID = c.Opt_ID))) where ((c.Opt_Type <> 'W') and (c.opt_url <> '...')) order by c.opt_url,b.OPT_REQ,a.ROLE_CODE ;


DROP VIEW IF EXISTS f_v_useroptdatascopes;
CREATE  VIEW f_v_useroptdatascopes AS select distinct a.USER_CODE AS User_Code,c.Opt_ID AS OPT_ID,c.OPT_METHOD AS OPT_METHOD,b.opt_Scope_Codes AS opt_Scope_Codes from ((f_v_userroles a join f_rolepower b on((a.ROLE_CODE = b.ROLE_CODE))) join f_optdef c on((b.OPT_CODE = c.OPT_CODE))) ;


DROP VIEW IF EXISTS f_v_useroptlist;
CREATE  VIEW f_v_useroptlist AS select distinct a.USER_CODE AS User_Code,c.OPT_CODE AS OPT_CODE,c.OPT_NAME AS OPT_NAME,c.Opt_ID AS OPT_ID,c.OPT_METHOD AS OPT_METHOD from ((f_v_userroles a join f_rolepower b on((a.ROLE_CODE = b.ROLE_CODE))) join f_optdef c on((b.OPT_CODE = c.OPT_CODE))) ;


DROP VIEW IF EXISTS f_v_useroptmoudlelist;
CREATE  VIEW f_v_useroptmoudlelist AS select distinct a.USER_CODE AS User_Code,d.Opt_ID AS Opt_ID,d.Opt_Name AS Opt_Name,d.Pre_Opt_ID AS Pre_Opt_ID,d.Form_Code AS Form_Code,d.opt_url AS opt_url,d.opt_Route AS opt_route,d.Msg_No AS Msg_No,d.Msg_Prm AS Msg_Prm,d.Is_In_ToolBar AS Is_In_ToolBar,d.Img_Index AS Img_Index,d.Top_Opt_ID AS Top_Opt_ID,d.Order_Ind AS Order_Ind,d.Page_Type AS Page_Type,d.Opt_Type AS opt_type from (((f_v_userroles a join f_rolepower b on((a.ROLE_CODE = b.ROLE_CODE))) join f_optdef c on((b.OPT_CODE = c.OPT_CODE))) join f_optinfo d on((c.Opt_ID = d.Opt_ID))) where (d.opt_url <> '...') ;


DROP VIEW IF EXISTS f_v_wf_optdef_url_map;
CREATE  VIEW f_v_wf_optdef_url_map AS select concat(c.opt_url,b.OPT_URL) AS optdef_url,b.OPT_REQ AS opt_req,b.OPT_CODE AS opt_code,b.OPT_DESC AS opt_desc,b.OPT_METHOD AS opt_Method,c.Opt_ID AS opt_id,b.OPT_NAME AS Opt_Name from (f_optdef b join f_optinfo c on((b.Opt_ID = c.Opt_ID))) where ((c.Opt_Type = 'W') and (c.opt_url <> '...') and (b.OPT_REQ is not null)) ;
...
CREATE  VIEW v_opt_tree AS select i.Opt_ID AS MENU_ID,i.Pre_Opt_ID AS PARENT_ID,i.Opt_Name AS MENU_NAME,i.Order_Ind AS order_ind from f_optinfo i where (i.Is_In_ToolBar = 'Y') union all select d.OPT_CODE AS MENU_ID,d.Opt_ID AS PARENT_ID,d.OPT_NAME AS MENU_NAME,0 AS order_ind from f_optdef d;
